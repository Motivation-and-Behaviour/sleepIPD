...
`sleepIPD` wraps `GGIR::g.shell.GGIR()` for the processing. If you are manually making calls to GGIR (i.e., not using this package), the settings are provide below. You **must** use GGIR Version 2.4-0 for this project. There is some evidence that different versions of GGIR can produce slightly different results. Please check your version before you begin (`utils::packageVersion("GGIR")`). 
