...
The project is led by [Dr Timothy Hartwig](mailto:Timothy.Hartwig@acu.edu.au), and this package is developed by [Dr Taren Sanders](Taren.Sanders@acu.edu.au). Please reach out if you need any support.
...
`sleepIPD` wraps `GGIR:g.shell.GGIR()` for the processing. If you are manually making calls to GGIR (i.e., not using this package), the settings are provide below. Please note that you **must** use GGIR Version 2.4-0. There is some evidence that GGIR can produce slightly different results for different version. Please check your version (`utils::packageVersion("GGIR")`). 
...
Note that `datadir`, `outputdir`, `overwrite`, and `studyname` are set elsewhere in `sleepIPD`. The thresholds (i.e., `threshold.lig`, `threshold.mod`, and `threshold.vig`) will depend on the device you are using, the age of the sample, and where the accelerometer was worn. See details below.
...
```{r echo=FALSE}
sleepIPD:::thresholds %>%
  mutate(source = map(source, gt::md)) %>%
  gt() %>%
  cols_label(
    age = "Age",
    dev = "Device",
    wear_loc = "Wear Location",
    light = "Light Threshold",
    mod = "Moderate Threshold",
    vig = "Vigorous Threshold",
    source = "Sources"
  ) %>%
  tab_style(style = cell_text(weight = "bold"),
            locations = cells_column_labels()) %>%
  cols_width(source ~ pct(35),
             everything() ~ pct(15)) %>%
  opt_row_striping() %>%
  tab_header("Acceleration Thresholds for the Pooled Sleep Study") %>%
  tab_footnote(footnote = html("Children: &le;18 years; Adults: > 18 years"),
               locations = cells_column_labels(columns = age)) %>% 
  tab_footnote(footnote = "Geneactiv thresholds can also be applied to Axivity devices.",
               locations = cells_column_labels(columns = dev))
```

Note that if you have data that would fall into more than one row (e.g., your sample includes both children and adults), you can add multiple cut-points and we will extract the right values later based on the metadata.
...
Even if you do not use `sleepIPD` for the reprocessing, you may still find the helper functions useful.
...
Note that if you have previously used `build_meta` and need to run it again, you need to specify `overwrite = TRUE`. This will replace the existing metadata template.
